...
#' @param n Single integer value indicating the number of colors required.
#' @param kind Single string. Either "std" for standard gg colors, "dark" for darkened gg colors, or "light" for pastel gg colors.
...
gg_palette <- function(
    ...
    tempo <- saferDev::arg_check(data = kind, class = NULL, typeof = NULL, mode = NULL, length = 1, prop = FALSE, double_as_integer_allowed = FALSE, options = base::c("std", "dark", "light"), all_options_in_data = FALSE, na_contain = FALSE, neg_values = TRUE, inf_values = TRUE, print = FALSE, data_name = NULL, data_arg = TRUE, safer_check = FALSE, lib_path = lib_path, error_text = embed_error_text) ; base::eval(expr = ee, envir = base::environment(fun = NULL), enclos = base::environment(fun = NULL)) # example of setting. Adapt to your own function or remove
    ...
    if(base::isTRUE(base::all.equal(n, 0))){ # isTRUE(all.equal(n, 0))) is similar to n == 0 but deals with float
            tempo_cat <- base::paste0(
            error_text_start, 
            "n ARGUMENT MUST BE A NON ZERO INTEGER. HERE IT IS: ", 
            base::paste(n, collapse = " ", recycle0 = FALSE), 
            collapse = NULL, 
            recycle0 = FALSE
        )
        base::stop(base::paste0("\n\n================\n\n", tempo_cat, "\n\n================\n\n", collapse = NULL, recycle0 = FALSE), call. = FALSE, domain = NULL)
